...
const UserPanel = () => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const res = await fetch(`app-7627139a-0e98-484c-b5e6-93e7f2612bf3.cleverapps.io/comments/users/${userData.id}`, {
          method: 'GET',
  ...
  const eliminarCuenta = async () => {
    ...
    try {
      const res = await fetch('app-7627139a-0e98-484c-b5e6-93e7f2612bf3.cleverapps.io/auth', {
        method: 'DELETE',
        headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await fetch('app-7627139a-0e98-484c-b5e6-93e7f2612bf3.cleverapps.io/auth/change-password', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${userData.token}`
